...
module.exports = function search(url, params, headers) {
  params || (params = {});
  ...
  const search = () => makeRequest(getInitialUrl(urlObj), body, headers);
  const scroll = nextRequestFn(getSubsequentUrl(urlObj), urlObj.query.scroll, headers);
